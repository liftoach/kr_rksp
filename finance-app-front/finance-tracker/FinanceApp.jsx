    ...
    <div style={styles.authContainer}>
      <div style={styles.authCard}>
        <div style={styles.authHeader}>
          ...
          <h1 style={styles.authTitle}>Finance Tracker</h1>
        </div>
...
function Sidebar({ currentPage, setCurrentPage, onLogout }) {
  const menuItems = [
    ...
    { id: 'transactions', label: 'Transactions', icon: 'ti-cash' },
    { id: 'categories', label: 'Categories', icon: 'ti-tags' },
    { id: 'budgets', label: 'Budgets', icon: 'ti-chart-bar' }
  ];
  ...
  return (
    <div style={styles.transactionRow}>
      <div style={styles.txInfo}>
        ...
        <p style={styles.txDescription}>{transaction.description || 'No description'}</p>
      </div>
      <p style={{...styles.txAmount, color}}>{sign}{transaction.amount}</p>
    </div>
  );
}
    ...
    <div style={styles.pageContent}>
      <div style={styles.pageHeader}>
        ...
        <button onClick={() => setShowForm(!showForm)} style={styles.primaryBtn}>
          <i className="ti ti-plus" style={styles.btnIcon}></i> Add Transaction
        </button>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={styles.formCard}>
          ...
          <div style={styles.formActions}>
            <button type="submit" style={styles.primaryBtn}>Save Transaction</button>
      ...
      {transactions?.length > 0 ? (
        ...
        <p style={styles.emptyState}>No transactions yet. Create one to get started!</p>
      )}
    </div>
  );
}
  ...
  return (
    <div style={styles.pageContent}>
      <div style={styles.pageHeader}>
        ...
        <button onClick={() => setShowForm(!showForm)} style={styles.primaryBtn}>
          <i className="ti ti-plus" style={styles.btnIcon}></i> New Category
        </button>
      </div>
          ...
          categories.map(cat => (
            <div key={cat.id} style={styles.categoryCard}>
              <div style={styles.categoryInfo}>
                ...
                <span style={{...styles.badge, backgroundColor: cat.transaction_type === 'IN' ? 'var(--color-background-success)' : 'var(--color-background-danger)'}}>
                  {cat.transaction_type === 'IN' ? 'Income' : 'Expense'}
                </span>
              </div>
              <button 
  ...
  return (
    <div style={styles.pageContent}>
      <div style={styles.pageHeader}>
        ...
        <button onClick={() => setShowForm(!showForm)} style={styles.primaryBtn}>
          <i className="ti ti-plus" style={styles.btnIcon}></i> Set Budget
        </button>
      </div>
      ...
      <div style={styles.budgetsList}>
        {budgets?.length > 0 ? (
          ...
          <p style={styles.emptyState}>No budgets set yet</p>
        )}
      </div>
    </div>
  );
